...
//                color = HEX("#24D06D")
                ...
                orderString = "劵后购买（¥" + String(format:"%.2f）", detilsOne.price - coupon.discountAmount) + "原价:¥\( detilsOne.price)"
    ...
    func courseDetilsOne() {
        ...
        NetWork.request(.courseDetilsOne,
            ...
            if let model = model {
                ...
                self.lesson.text = "共\(model.pointCount)讲，已更新\(model.updatePointCount)讲"
            }
        }
    }
...
extension Course2VC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.name.textColor = indexPath == self.indexPath ? HEX(kMainColor1) : UIColor.black
        return cell
    }
    ...
    func submitStudyRecordDirectory(completion: (() -> Void)? = nil, _ force: Bool = false) {
        if let section = section, let row = row, (progress >= 0 || force){
    ...
    func getPlayInfoWithOptions(_ videoId: String, _ studyBili: String) {
    ...
    func showVideo(_ url: URL, _ studyBili: String) {
        ...
        observer = player.addPeriodicTimeObserver(forInterval: CMTime.init(value: 1, timescale: CMTimeScale(NSEC_PER_SEC)), queue: nil, using: {(cmtime) in
            if first {
                if self.video != nil && Double(studyBili) ?? 0 < 1 {
                    self.submitStudyRecordDirectory(completion: nil, true)
                }
                let timeStart = CMTimeGetSeconds(player.currentItem!.duration) * (Double(studyBili) ?? 0) / 100
    ...
    func study(_ selectedSection: GetCourseDetilsOneList, _ item: GetCourseDetilsItem) {
        ...
        if !item.aliyunVideoId.isEmpty,
           item.category == "1"
        {
            getPlayInfoWithOptions(item.aliyunVideoId, item.studyBili)
...
//            player = AliPlayer.init()
//            let source = AVPUrlSource.init().url(with: item.videoUrl)
//            player?.setUrlSource(source)
//
//            player?.playerView = banner.superview
//            player?.prepare()
//            player?.start()
